...
Generated at: 2025-04-01 18:30:28
...
def calculate_total_sales(sales_data: List[Dict[str, Union[str, float]]]) -> Dict[str, float]:
    ...
    Raises:
    ValueError: If any sales data is invalid (e.g., 'sales' is not a float or 'product' is not a string).
    """

    # Initialize an empty dictionary to store the total sales for each product
    total_sales = {}

    # Iterate over each sales record in the sales_data list
    for record in sales_data:
        # Extract the product name and sales amount from the record
        ...
        if not isinstance(product, str) or not isinstance(sales, (int, float)):
            raise ValueError(f"Invalid sales data: {record}")

        # If the product is already in the dictionary, add the sales to the existing total
        if product in total_sales:
            total_sales[product] += sales
        else:
            # Otherwise, initialize the total sales for this product
            total_sales[product] = sales

    # Return the dictionary with total sales for each product
    return total_sales

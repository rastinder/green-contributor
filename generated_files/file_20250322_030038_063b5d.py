...
Generated at: 2025-03-22 03:00:42
...
def process_csv_data(file_path: str) -> Dict[str, Dict[str, Union[float, str]]]:
    """
            ...
            for row in reader:
                for column in data:
                    try:
                        ...
                        # Handle non-numeric data by skipping the value
                        continue

            # Calculate mean and standard deviation for each column
            result = {}
            for column, values in data.items():
                if values:  # Ensure the column has numeric data
                    mean = statistics.mean(values)
                    std_dev = statistics.stdev(values) if len(values) > 1 else 0.0
                    result[column] = {'mean': mean, 'std_dev': std_dev}
                else:
                    result[column] = {'mean': 'N/A', 'std_dev': 'N/A'}

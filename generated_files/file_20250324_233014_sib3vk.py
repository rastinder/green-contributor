...
Generated at: 2025-03-24 23:30:19
...
def process_and_analyze_data(file_path: str, column_name: str) -> Tuple[float, float]:
    """
    ...
    and then calculates the mean and standard deviation of the specified column.
    ...
    try:
        ...
        data_cleaned[column_name] = pd.to_numeric(data_cleaned[column_name], errors='coerce')

        # Drop rows where the conversion to numeric failed (i.e., non-numeric values)
        data_cleaned = data_cleaned.dropna(subset=[column_name])

        # Calculate the mean and standard deviation of the specified column
        mean = data_cleaned[column_name].mean()
        std_dev = data_cleaned[column_name].std()

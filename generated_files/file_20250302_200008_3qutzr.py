...
Generated at: 2025-03-02 20:00:13
...
def process_csv(file_path: str) -> Union[Dict[str, Tuple[float, float]], str]:
    ...
    Returns:
        ...
        containing the mean and standard deviation, or an error message if an error occurs.
    ...
    try:
        ...
        with open(file_path, mode='r', newline='') as file:
            ...
            for row in reader:
                for column, value in row.items():
                    try:
                        ...
                        results[column][0].append(float_value)
                    except ValueError:
            ...
            for column in results:
                values = results[column][0]
                if len(values) > 1:
                    mean = statistics.mean(values)
                    stdev = statistics.stdev(values)
                    results[column] = (mean, stdev)
                else:
                    results[column] = (values[0], 0.0)
        ...
        return "Error: The file is not in the correct CSV format."

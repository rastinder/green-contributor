...
Generated at: 2025-04-02 10:00:25
...
def calculate_total_sales(sales_data: List[Dict[str, Union[str, float]]]) -> Dict[str, float]:
    ...
    for record in sales_data:
        # Check if the record is a dictionary and contains the required keys
        if not isinstance(record, dict) or 'product' not in record or 'sales' not in record:
            raise ValueError("Each record must be a dictionary with 'product' and 'sales' keys.")

        # Extract product name and sales amount from the record
        product = record['product']
        sales = record['sales']

        # Ensure the product name is a string and sales amount is a float
...
sales_data = [
    ...
    {'product': 'Banana', 'sales': 50.75}
]

try:
    result = calculate_total_sales(sales_data)
    print(result)  # Output: {'Apple': 250.75, 'Banana': 250.75}

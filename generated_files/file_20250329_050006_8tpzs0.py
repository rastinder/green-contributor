...
Generated at: 2025-03-29 05:00:12
...
def calculate_total_sales(sales_data: List[Dict[str, Union[str, float]]]) -> Dict[str, float]:
    ...
    Args:
        sales_data (List[Dict[str, Union[str, float]]]): A list of dictionaries where each dictionary
            contains the keys 'product' (str) and 'sales' (float).

    Returns:
        Dict[str, float]: A dictionary with product names as keys and their total sales as values.

    Raises:
        ValueError: If any entry in sales_data does not contain 'product' and 'sales' keys.
        TypeError: If 'sales' value is not a float or 'product' value is not a string.
    """
    total_sales = {}

    for entry in sales_data:
        # Check if the required keys are present
        if 'product' not in entry or 'sales' not in entry:
            raise ValueError("Each entry must contain 'product' and 'sales' keys.")
        ...
        if not isinstance(product, str):
            raise TypeError(f"Expected 'product' to be a string, got {type(product).__name__}.")
        if not isinstance(sales, (float, int)):
            raise TypeError(f"Expected 'sales' to be a float or int, got {type(sales).__name__}.")

...
Generated at: 2025-03-28 14:00:07
...
def calculate_statistics(df: pd.DataFrame) -> Dict[str, Union[float, Dict[str, float]]]:
    """
    ...
    Returns:
    Dict[str, Union[float, Dict[str, float]]]: A dictionary containing the calculated statistics.
    The keys are 'mean', 'median', and 'std', and the values are dictionaries with column names as keys
    and the corresponding statistics as values.
    ...
    mean = numerical_columns.mean().to_dict()
    median = numerical_columns.median().to_dict()
    std = numerical_columns.std().to_dict()
